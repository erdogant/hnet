""" This function creates an interactive network using D3 javascript and html.
 ...
 EXAMPLE
   ...
   for i in range(0,len(G.node)):
       node_color.append(G.node[i]['club'])
   node_name=node_color

   A = d3graph(adjmat, df=df, node_color=node_size, node_size=node_size, node_name=node_name, path='c://temp/magweg/')
...
def set_source_dir(config):
    ...
    if not os.path.isdir(config['path']) and config['path']!='':
        print('[D3GRAPH] Creating directory: %s' %(config['path']))
