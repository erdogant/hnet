...
def d3graph(model, node_size_limits=[6,15], savepath=None, node_color=None, directed=True, showfig=True):
    ...
    # Make network
    simmatLogP = to_symmetric(model, method='logp')
    # Make network
...
# %% Make network d3
def plot(model, scale=2, dist_between_nodes=0.4, node_size_limits=[25,500], node_color=None, savepath=None, figsize=[15,10], pos=None, layout='fruchterman_reingold', dpi=250, showfig=True):
    ...
    ----------
    model : dict
        model that is the output of the fit() function.
    scale : int, optional
...
def to_symmetric(out, method='logp', verbose=3):
    ...
    """
    if np.sum(np.isin(out['simmatLogP'].index.values, out['simmatLogP'].columns.values))!=np.max(out['simmatLogP'].shape):
