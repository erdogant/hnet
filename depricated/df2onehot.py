...
def df2onehot(df, dtypes='pandas', hot_only=True, y_min=None, list_expand=True, perc_min_num=None, excl_background=None, verbose=3):
    ...
    # Determine Dtypes
    [df, dtypes] = df2onehot.set_dtypes(df, config['dtypes'], is_list=config['list_expand'], perc_min_num=config['perc_min_num'], verbose=config['verbose'])
...
#                df1.iloc[idxRow[k],I]=True
    ...
    # Redo the typing
    [df, dtypes] = df2onehot.set_dtypes(df, verbose=0)
    # Return
    return(df, dtypes)
